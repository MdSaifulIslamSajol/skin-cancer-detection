...
import sys
sys.stdout = open(f"./results/console_output_lr{learning_rate}_bs{batch_size}_inference_time.txt", "w")
...
def train_test_plot(model_name = 'resnet'):
    ...
    import time  # Add this import to use the time module

    def validate(val_loader, model, criterion, optimizer, epoch):
        model.eval()
        val_loss = AverageMeter()
        val_acc = AverageMeter()
        total_time = 0  # Variable to accumulate the total inference time
        total_images = 0  # Variable to accumulate the total number of images processed
        ...
        avg_inference_time = total_time / total_images  # Calculate average inference time per image
        
        print('[epoch %d], [val loss %.5f], [val acc %.5f], [avg inference time per image %.5f seconds]' % (epoch, val_loss.avg, val_acc.avg, avg_inference_time))
        return val_loss.avg, val_acc.avg, avg_inference_time
    ...
    for epoch in range(1, epochs + 1):
        ...
        loss_val, acc_val, avg_inference_time = validate(val_loader, model, criterion, optimizer, epoch)
        ...
        if acc_val > best_val_acc:
            ...
            print('best record: [epoch %d], [val loss %.5f], [val acc %.5f], [avg inference time per image %.5f seconds]' % (epoch, loss_val, acc_val, avg_inference_time))
            print('*****************************************************')
    ...
    plt.tight_layout()
    plt.savefig(f"./results/{model_name}_lr{learning_rate}_bs{batch_size}_loss_curve.png", dpi=300)  # Save the figure with high resolution
    ...
    plt.tight_layout()
    plt.savefig(f"./results/{model_name}_lr{learning_rate}_bs{batch_size}_accuracy_curve.png", dpi=300)  # Save the figure with high resolution
    ...
    plt.savefig(f"./results/{model_name}_lr{learning_rate}_bs{batch_size}_confusion_matrix.png", dpi=300)  # Save the figure with high resolution
    plt.show()
...
train_test_plot(model_name= "resnet")
train_test_plot(model_name= "vgg")
train_test_plot(model_name= "densenet")
train_test_plot(model_name= "googlenet")
train_test_plot(model_name= "efficientnet")
train_test_plot(model_name= "mobilenet")
